...
function get_isolated_random_prime_api_results(dataReceived) {
    ...
    for (let key in dataReceived) {
        ...
        if (typeof dataReceived[key] == "object") {
            ...
            buildHtmlResults += `<td class="${key}">${get_isolated_random_prime_api_results_details(dataReceived[key], key)}</td>`;
            buildHtmlResults += `</tr>`;
        }
        else {

$(document).ready(function () {
    ...
    function get_random_prime_api_call(get_random_prime_apiKey, get_random_prime_check_start, get_random_prime_check_end, get_random_prime_include_explanations, get_random_prime_include_prime_types_list, get_random_prime_language) {
        ...
        $.ajax({
            url: get_random_prime_api_url,
            type: 'get',
            cache: false,
            success: function (dataReceived) {
                console.log('success!');
                get_random_prime_api_results(dataReceived);
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                console.log("Status: ", textStatus); 
                console.log("Error: ", errorThrown);
                console.log("XMLHttpRequest: ", XMLHttpRequest.responseJSON.error);
                displayError(XMLHttpRequest.responseJSON.error, "error-get-random-prime")
            }
        });
    };
})
...
function get_random_prime_api_results(dataReceived) {
    ...
    for (let key in dataReceived) {
        ...
        if (typeof dataReceived[key] == "object") {
            ...
            buildHtmlResults += `<td class="${key}">${get_random_prime_api_results_details(dataReceived[key], key)}</td>`;
            buildHtmlResults += `</tr>`;
        }
        else {
...
function get_random_prime_api_results_details(dataReceived, parentKey) {
